    ...
    <Collapsible open={isOpen} onOpenChange={setIsOpen}>
      <Card className="bg-slate-800/30 border-slate-700/50">
        <CollapsibleTrigger asChild>
          <button className="w-full px-5 py-4 flex items-center justify-between hover:bg-slate-700/20 transition-colors rounded-lg">
            <div className="flex items-center gap-3">
              <div className="p-2 rounded-lg bg-indigo-500/10">
                <Info className="h-4 w-4 text-indigo-400" />
              </div>
              <div className="text-left">
                <h3 className="text-sm font-medium text-slate-200">Notes de Calcul</h3>
                <p className="text-xs text-slate-500">Hypothèses et formules utilisées</p>
              </div>
            </div>
            <ChevronDown 
              className={cn(
        ...
        <CollapsibleContent>
          <CardContent className="pt-0 pb-5 px-5">
            <div className="border-t border-slate-700/50 pt-4 space-y-6">
              ...
              <div>
                ...
                <div className="space-y-2">
                  {formulas.map((item, idx) => (
                    <div 
                      key={idx}
                      className="flex items-center justify-between bg-slate-900/50 rounded-lg px-4 py-3 border border-slate-700/30"
                    >
                      <div className="flex items-center gap-4">
                        <span className="text-sm font-medium text-slate-200">{item.name}</span>
                        <code className="text-xs bg-slate-800 text-emerald-400 px-2 py-1 rounded font-mono">
                          {item.formula}
                        </code>
                      </div>
              ...
              {/* Sources */}
              <div className="text-xs text-slate-500 bg-slate-900/30 rounded-lg p-3 border border-slate-700/20">
                <strong className="text-slate-400">Sources :</strong> Tarifs STEG 2024, Commission Européenne (CBAM), 
